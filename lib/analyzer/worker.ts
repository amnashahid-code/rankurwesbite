...
export async function runScan(id?:string) {
 ...
 }catch(e){await db.from('website_scans').update({status:'failed',stage:'Scan could not finish',error:e instanceof Error ? e.message.slice(0,250):'Scan failed.'}).eq('id',scan.id).eq('status','running').eq('attempts',scan.attempts);}
 return true;
}
